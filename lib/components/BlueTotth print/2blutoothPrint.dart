...
class BluePrint {
  ...
  Future<List<int>> salesBill(Map<String, dynamic> printSalesData,
      String payment_mode, String iscancelled, double bal) async {
    // String balance = printSalesData["master"]["ba"].toString();
    // double balnc = double.parse(balance);
    print("entered sales bill");
    List<int> bytes = [];
    CapabilityProfile profile = await CapabilityProfile.load();
    final generator = Generator(PaperSize.mm80, profile);
    bytes += generator.text('ESTIMATE',
        styles: PosStyles(
        ...
        linesAfter: 1);
    bytes +=
        generator.text('CREDIT', styles: PosStyles(align: PosAlign.center));
